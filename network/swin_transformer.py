...
class BasicLayers(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                mlp_ratio, qkv_bias, qk_scale=None, drop=0, attn_drop=0.,
                drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False) -> None:
        super().__init__()
        self.dim = dim
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            x = torch.roll(shift_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            x = shift_x
        x = x.view(B, H * W, C)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                ...
                use_checkpoint=False, **kwargs) -> None:
        super().__init__()
        ...
        if self.ape:
            self.absolute_position_embed = nn.Parameter(torch.zeros(1, self.num_patches, self.embed_dim))
            trunc_normal_(self.absolute_position_embed, std=.02)
    ...
    def forward_feature(self, x):
        ...
        if self.ape:
            x = x + self.absolute_position_embed
        x = self.pos_drop(x)
